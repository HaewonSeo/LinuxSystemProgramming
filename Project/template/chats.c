...
ProcessClient(int id)
{
	...
	//set the state and type of cancellation request 
	//enable & asynchronous
	...
	while (1)  {
		...
		if (n == 0)  {
			...
			close(Client[id].sockfd);			//clode the sockfd
			Client[id].inUse = 0;				//initialize the client[id]
...
CloseServer(int signo)
{
	...
	for (i = 0 ; i < MAX_CLIENT ; i++)  {
		if (Client[i].inUse)  {
			...
			//wait for terimation of threads 
