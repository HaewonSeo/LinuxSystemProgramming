...
main()
{
	...
	// initialize the mutex & condition semaphore of EmptySem
	// initial value : MAX_BUF
	if (sem_init(&EmptySem, 0, MAX_BUF) < 0)  {
		perror("sem_init");
		exit(1);
	}
	// initialize the mutex & condition semaphore of FullSem
	// initial value : 0
	if (sem_init(&FullSem, 0, 0) < 0)  {
		perror("sem_init");
		exit(1);
	}
	// initialize the mutex & condition semaphore of MutexSem
	// initial value : 1
